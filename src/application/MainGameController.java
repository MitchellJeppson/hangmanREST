...
public class MainGameController {
	...
	private Image[] hangmanImageArray = {new Image("start.jpg"), new Image("head.jpg"), new Image("body.jpg"), new Image("leftLeg.jpg"), new Image("rightLeg.jpg"), new Image("leftArm.jpg"), new Image("rightArm.jpg"), new Image("leftEye.jpg"), new Image("rightEye.jpg"), new Image("mouth.jpg")};

	@FXML
	void initialize(){
		hangingImageView.setImage(hangmanImageArray[0]);
		...
		int num = new SecureRandom().nextInt(194434); //number of lines in words.txt
		int count = 1;
	...
	void keyboardInput(KeyEvent e){
		...
		if(guess.compareTo("A") >= 0 && guess.compareTo("Z") <= 0){								//guess is a letter
			if(!oldGuesses.contains(guess)){														//guess is a new letter
				countOfGuesses++;
				if(hiddenWord.toUpperCase().contains(guess)){									//guess is in guessingWord
					...
					if(imageIndex != 9){
						imageIndex++;
						hangingImageView.setImage(hangmanImageArray[imageIndex]);
					}
					else{
						gameOver(false);
					}
	...
	private void gameOver(boolean wonTheGame){
		if(wonTheGame){
			...
			d.setHeaderText("Congratulations!");
			d.setContentText("Please enter your name: ");
			Optional<String> answer = d.showAndWait();
			if(answer.isPresent()){
				try{
					URL obj = new URL("http", "localhost", 8081, "/hangman/add");			//Connect to rest server with name and score as post parameters.
